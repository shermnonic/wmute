...
struct GLGrabFramebuffer
{
	...
	: data(NULL),size(0),bytes_per_pixel(0)
	{
		...
		if( opts & FromTexture )
		{
			viewport[0] = 0;
			viewport[1] = 0;
			glGetTexLevelParameteriv( GL_TEXTURE_2D, 0, GL_TEXTURE_WIDTH,  &viewport[2] );
			glGetTexLevelParameteriv( GL_TEXTURE_2D, 0, GL_TEXTURE_HEIGHT, &viewport[3] );
		...
		if( opts & OverrideSize && overrideWidth>0 && overrideHeight>0 )
		{
			viewport[2] = overrideWidth;
			viewport[3] = overrideHeight;
		}
		...
		GL::checkGLError("GLGrabFramebuffer()");
	}
			  ...
			  int width, int height )
{
	...
	if( file.is_open() )
	{
		GLGrabFramebuffer* grabfb = NULL;

		int opts = GLGrabFramebuffer::ForceWidthMultiple4;
		if( fromActiveTexture )
			opts |= GLGrabFramebuffer::FromTexture;
		if( width>0 && height>0 ) 
		{
			opts |= GLGrabFramebuffer::OverrideSize;
			grabfb = new GLGrabFramebuffer( GL_BGR, opts, sizeof(TGA_HEADER),
				                            width, height );
		}
		else
			grabfb = new GLGrabFramebuffer( GL_BGR, opts, sizeof(TGA_HEADER) );
...
Screenshot2::Screenshot2()
	: m_initialized(false),
      ...
      m_texid(-1), m_texunit(10), m_fbo(-1)
{}
