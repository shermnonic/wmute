...
string parseShaderDefine( string line, string comment,
						  ShaderPrecompiler::ShaderDefine& def, 
						  const ShaderPrecompiler::ShaderDefines& defs,
						  bool& ok )
{
	vector<string> tokens;
...
string parseShaderVariable( string line, string comment, 
						    ShaderPrecompiler::ShaderVariable& var, bool& ok )
{
	// Replace '=' with ' '
	size_t p;
	...
	{			
		// Store variable
		var = ShaderPrecompiler::ShaderVariable( sType, sName, sDefault );
...
std::string ShaderPrecompiler
  ::precompile( const std::string& shader, ShaderDefines& defines )
{
	using namespace std;
