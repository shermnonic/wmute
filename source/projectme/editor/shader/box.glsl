...
vec3 BoxBlur5( sampler2D tex, vec2 uv, vec2 ofs )
{
	return (1.0/25.0)*(
...
vec3 BoxBlur3( sampler2D tex, vec2 uv, vec2 ofs )
{
	return (1.0/9.0)*(
	texture2D(tex,uv+vec2(-1.0,-1.0)*ofs).rgb +
	texture2D(tex,uv+vec2( 0.0,-1.0)*ofs).rgb +
...
void main()
{
	...
	gl_FragColor = vec4( BoxBlur5( iChannel0, uv, vec2(1.0)/iChannelResolution[0].xy ), 1.0 );
