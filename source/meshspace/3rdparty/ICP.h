...
namespace SICP {
	...
	/// Closest points (provided for convenience to get closest points *after*
	/// ICP computation). Introduces a slight overhead by recomputing the kD tree
	/// but leaves the SICP interface intact.
	/// @author Max Hermann (hermann@cs.uni-bonn.de)
	template <typename Derived1, typename Derived2>
	void closest_points(const Eigen::MatrixBase<Derived1>& X,
						...
						std::vector<int>& ids )
	{
        ...
        #pragma omp parallel for
        for(int i=0; i<X.cols(); ++i) {
            ids[i] = kdtree.closest(X.col(i).data());
        }
	}
    ...
    void point_to_point(Eigen::MatrixBase<Derived1>& X,
        ...
        for(icp=0; icp<par.max_icp; ++icp) {
            ...
            Xo2 = X;
			if( par.callback ) par.callback( icp, stop );
            if(stop < par.stop) break;
        }
		/// Output shrinkage matrix
		if( Z_out ) *Z_out = Z;
    }
    ...
    void point_to_plane(Eigen::MatrixBase<Derived1>& X,
        ...
        for(int icp=0; icp<par.max_icp; ++icp) {
            ...
            for(int outer=0; outer<par.max_outer; ++outer) {
                ...
                /// C update (lagrange multipliers)
                //Eigen::VectorXf 
				Eigen::Matrix3Xd P = (Qn.array()*(X-Qp).array()).colwise().sum().transpose()-Z.array();
                if(!par.use_penalty) C.noalias() += mu*P;
                /// mu update (penalty)
                if(mu < par.max_mu) mu *= par.alpha;
                /// Stopping criteria
